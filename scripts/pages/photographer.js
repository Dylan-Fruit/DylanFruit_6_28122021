...
async function displayMedias(media) {
  ...
  function toggleList() {
    if (
      !sortByList.getAttribute("style") ||
      sortByList.getAttribute("style") === "display: none;"
    ) {
  ...
  const previous = () => {
    let lightboxMediaCard = document.querySelector(
      ".lightbox-slider_medias"
    ).firstElementChild;

    const result = dispMedias.find(
      (element) => element.id === parseInt(lightboxMediaCard.dataset.id, 10)
    );
    ...
    lightboxMediaCard = document.querySelector(
      ".lightbox-slider_medias"
    ).firstElementChild;
  };
